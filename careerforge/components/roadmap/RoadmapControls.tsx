...
export const RoadmapControls: React.FC<RoadmapControlsProps> = ({
  ...
  return (
    <div className="space-y-4 mb-6">
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs font-medium transition-all cursor-pointer whitespace-nowrap border ${
                isSelected
                  ? "bg-[#141824] text-white border-slate-700 shadow-sm ring-1 ring-white/10"
                  : "bg-[#0d1017]/80 text-slate-400 border-slate-800/80 hover:bg-[#121520] hover:text-slate-200"
              }`}
            >
              <span
      ...
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 p-3 rounded-xl bg-[#101420]/90 border border-slate-800/80 shadow-sm backdrop-blur-md">
        ...
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full pl-8 pr-12 py-1.5 rounded-lg bg-[#0c0f18] border border-slate-800 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400/30 transition-all"
          />
          ...
          {isSearchFocused && searchSuggestions.length > 0 && (
            <div className="absolute top-full left-0 right-0 mt-1 rounded-xl bg-[#121624] border border-slate-800 shadow-xl z-50 overflow-hidden py-1">
              ...
              {searchSuggestions.map((node) => (
                <div
                  ...
                  className="flex items-center justify-between px-3 py-2 hover:bg-[#1a2032] cursor-pointer text-left transition-colors"
                >
                  <div>
        ...
        <div className="flex items-center gap-3 flex-wrap sm:flex-nowrap">
          {/* Status Filters */}
          <div className="inline-flex rounded-lg bg-[#0c0f18] p-1 border border-slate-800 shrink-0">
            {(
              [
                { id: "all", label: "All" },
          ...
          {onResetProgress && (
            <button
              ...
              className="p-1.5 rounded-lg text-slate-500 hover:text-rose-400 hover:bg-slate-800 transition-colors cursor-pointer text-xs"
            >
              ↺ Reset
            </button>
          )}
